...
public class CombinedBindListHudElement extends ListHudElement {
    ...
    private final BooleanSetting hideActive = new BooleanSetting("HideActive", "Hides active modules with keybinds", false);
    private final BooleanSetting hideInactive = new BooleanSetting("HideInactive", "Hides inactive modules with keybinds", false);
    ...
    private final ColorSetting mBoundColor = new ColorSetting("MBound", "Color for metadata text of bound modules", Color.YELLOW)
            .setRainbowAllowed(true).setThemeSyncAllowed(true);
    private final ColorSetting mUnboundColor = new ColorSetting("MUnbound", "Color for metadata text of unbound modules", Color.BLUE)
            .setRainbowAllowed(true).setThemeSyncAllowed(true);
    private final ColorSetting mBrackets = new ColorSetting("MBrackets", "Color for metadata brackets of bound modules", Color.YELLOW)
            .setRainbowAllowed(true).setThemeSyncAllowed(true);
            ...
            if (!hideBounded.getValue()) {
                for (ModuleHolder module : modules) {
                    if (module != null && module.hasKeybind() && module.isVisible()) {
                        if (hideActive.getValue() && module.isEnabled()) continue;
                        if (hideInactive.getValue() && !module.isEnabled()) continue;
    ...
    static class ModuleHolder {
        ...
        public String getKeybind() {
            try {
                if (moduleType == ModuleType.METEOR && meteorModule != null && meteorModule.keybind != null) {
                    return getMeteorKeybindWithModifiers();
                } else if (moduleType == ModuleType.RUSHER && rusherModule != null) {
                    return getRusherKeybindWithModifiers();
                }
            } catch (Exception ignored) {
            }
            return "unbound";
        ...
        public String getKeybindRaw() {
            try {
                if (moduleType == ModuleType.METEOR && meteorModule != null && meteorModule.keybind != null) {
                    return getMeteorKeybindRawWithModifiers();
                } else if (moduleType == ModuleType.RUSHER && rusherModule != null) {
                    return getRusherKeybindRawWithModifiers();
                }
            } catch (Exception ignored) {
            }
            return "unbound";
        ...
        private String getMeteorKeybindWithModifiers() {
            try {
                var keybind = meteorModule.keybind;
                StringBuilder result = new StringBuilder();

                boolean hasCtrl = false, hasShift = false, hasAlt = false;

                String[] ctrlNames = {"ctrl", "control", "ctrlPressed", "controlPressed"};
                String[] shiftNames = {"shift", "shiftPressed"};
                String[] altNames = {"alt", "altPressed"};

                for (String name : ctrlNames) {
                    try {
                        Field field = keybind.getClass().getDeclaredField(name);
                        field.setAccessible(true);
                        hasCtrl = field.getBoolean(keybind);
                        if (hasCtrl) break;
                    } catch (Exception ignored) {}
                }
                ...
                for (String name : shiftNames) {
                    try {
                        Field field = keybind.getClass().getDeclaredField(name);
                        field.setAccessible(true);
                        hasShift = field.getBoolean(keybind);
                        if (hasShift) break;
                    } catch (Exception ignored) {}
                }
                ...
                for (String name : altNames) {
                    try {
                        Field field = keybind.getClass().getDeclaredField(name);
                        field.setAccessible(true);
                        hasAlt = field.getBoolean(keybind);
                        if (hasAlt) break;
                    } catch (Exception ignored) {}
                }
                ...
                if (!hasCtrl || !hasShift || !hasAlt) {
                    try {
                        Method isCtrlMethod = keybind.getClass().getMethod("isCtrlPressed");
                        hasCtrl = (boolean) isCtrlMethod.invoke(keybind);
                    } catch (Exception ignored) {}

                    try {
                        Method isShiftMethod = keybind.getClass().getMethod("isShiftPressed");
                        hasShift = (boolean) isShiftMethod.invoke(keybind);
                    } catch (Exception ignored) {}

                    try {
                        Method isAltMethod = keybind.getClass().getMethod("isAltPressed");
                        hasAlt = (boolean) isAltMethod.invoke(keybind);
                    } catch (Exception ignored) {}
                }

                if (!hasCtrl && !hasShift && !hasAlt) {
                    try {
                        Method getModifiersMethod = keybind.getClass().getMethod("getModifiers");
                        int modifiers = (int) getModifiersMethod.invoke(keybind);
                        hasCtrl = (modifiers & 2) != 0;
                        hasShift = (modifiers & 1) != 0;
                        hasAlt = (modifiers & 4) != 0;
                    } catch (Exception ignored) {}
                }

                if (!hasCtrl && !hasShift && !hasAlt) {
                    String keybindStr = keybind.toString().toLowerCase();
                    hasCtrl = keybindStr.contains("ctrl") || keybindStr.contains("control");
                    hasShift = keybindStr.contains("shift");
                    hasAlt = keybindStr.contains("alt");
                }

                if (!hasCtrl && !hasShift && !hasAlt) {
                    Field[] fields = keybind.getClass().getDeclaredFields();
                    for (Field field : fields) {
                        try {
                            field.setAccessible(true);
                            if (field.getType() == boolean.class) {
                                boolean value = field.getBoolean(keybind);
                                if (value) {
                            ...
                            } else if (field.getType() == int.class && field.getName().toLowerCase().contains("modifier")) {
                                int modifiers = field.getInt(keybind);
                                hasCtrl = (modifiers & 2) != 0;
                                hasShift = (modifiers & 1) != 0;
                                hasAlt = (modifiers & 4) != 0;
                            }
                ...
                String mainKey = getMeteorKeybind();
                if (!mainKey.equals("unbound")) {
                    String lowerMainKey = mainKey.toLowerCase();
                    boolean mainKeyHasCtrl = lowerMainKey.contains("ctrl") || lowerMainKey.contains("control");
                    boolean mainKeyHasShift = lowerMainKey.contains("shift");
                    boolean mainKeyHasAlt = lowerMainKey.contains("alt");

                    if (hasCtrl && !mainKeyHasCtrl) result.append("Ctrl + ");
                    if (hasShift && !mainKeyHasShift) result.append("Shift + ");
                    if (hasAlt && !mainKeyHasAlt) result.append("Alt + ");
        ...
        private String getMeteorKeybindRawWithModifiers() {
            try {
                var keybind = meteorModule.keybind;
                StringBuilder result = new StringBuilder();
                ...
                if (rawKeybind != null && !rawKeybind.isEmpty() && !rawKeybind.equals("unbound")) {
                    ...
                    String lowerRawKeybind = rawKeybind.toLowerCase();
                    boolean rawHasCtrl = lowerRawKeybind.contains("ctrl") || lowerRawKeybind.contains("control");
                    boolean rawHasShift = lowerRawKeybind.contains("shift");
                    boolean rawHasAlt = lowerRawKeybind.contains("alt");

                    if (hasCtrl && !rawHasCtrl) result.append("CTRL+");
                    if (hasShift && !rawHasShift) result.append("SHIFT+");
                    if (hasAlt && !rawHasAlt) result.append("ALT+");

                    result.append(rawKeybind);
                    return result.toString();
                }
        ...
        private String getRusherKeybindWithModifiers() {
            try {
                final IKey key = RusherHackAPI.getBindManager().getBindRegistry().get(rusherModule);
                if (key != null) {
                    StringBuilder result = new StringBuilder();
                    ...
                    try {
                        Field modifiersField = key.getClass().getDeclaredField("modifiers");
                        modifiersField.setAccessible(true);
                        int modifiers = modifiersField.getInt(key);
                        hasCtrl = (modifiers & 2) != 0;
                        hasShift = (modifiers & 1) != 0;
                        hasAlt = (modifiers & 4) != 0;
                    } catch (Exception ignored) {}
                    ...
                    if (!hasCtrl || !hasShift || !hasAlt) {
                        try {
                            Method hasModifierMethod = key.getClass().getMethod("hasModifier", int.class);
                            hasCtrl = (boolean) hasModifierMethod.invoke(key, 2);
                            hasShift = (boolean) hasModifierMethod.invoke(key, 1);
                            hasAlt = (boolean) hasModifierMethod.invoke(key, 4);
                        } catch (Exception ignored) {}
                    }
                        ...
                        for (Field field : fields) {
                            try {
                                ...
                                if (field.getType() == int.class) {
                                    String fieldName = field.getName().toLowerCase();
                                    if (fieldName.contains("modifier")) {
                                        int modifiers = field.getInt(key);
                                        hasCtrl = (modifiers & 2) != 0;
                                        hasShift = (modifiers & 1) != 0;
                                        hasAlt = (modifiers & 4) != 0;
                                        break;
                                    ...
                                    boolean value = field.getBoolean(key);
                                    if (value) {
                                        String fieldName = field.getName().toLowerCase();
                                        if (fieldName.contains("ctrl") || fieldName.contains("control")) {
                                            hasCtrl = true;
                    ...
                    if (!hasCtrl && !hasShift && !hasAlt) {
                        try {
                            String fullLabel = key.getLabel(false);
                            if (fullLabel != null) {
                                String lowerLabel = fullLabel.toLowerCase();
                                hasCtrl = lowerLabel.contains("ctrl") || lowerLabel.contains("control");
                                hasShift = lowerLabel.contains("shift");
                                hasAlt = lowerLabel.contains("alt");
                            }
                        } catch (Exception ignored) {}
                    }
                    ...
                    if (!hasCtrl && !hasShift && !hasAlt) {
                        try {
                            ...
                            String keyName = (String) getNameMethod.invoke(key);
                            if (keyName != null) {
                                String lowerName = keyName.toLowerCase();
                                hasCtrl = lowerName.contains("ctrl") || lowerName.contains("control");
                                hasShift = lowerName.contains("shift");
                                hasAlt = lowerName.contains("alt");
                            }
                        } catch (Exception ignored) {
                            String keyStr = key.toString().toLowerCase();
                            hasCtrl = keyStr.contains("ctrl") || keyStr.contains("control");
                            hasShift = keyStr.contains("shift");
                            hasAlt = keyStr.contains("alt");
                        }
                    }

                    String rawLabel = key.getLabel(true);
                    String mainKey = formatRusherKeybind(rawLabel);
                    if (!mainKey.equals("unbound")) {
                        String lowerMainKey = mainKey.toLowerCase();
                        boolean mainKeyHasCtrl = lowerMainKey.contains("ctrl") || lowerMainKey.contains("control");
                        boolean mainKeyHasShift = lowerMainKey.contains("shift");
        ...
        private String getRusherKeybindRawWithModifiers() {
            try {
                final IKey key = RusherHackAPI.getBindManager().getBindRegistry().get(rusherModule);
                if (key != null) {
                    StringBuilder result = new StringBuilder();
                    ...
                    if (rawLabel != null && !rawLabel.isEmpty() && !rawLabel.equals("unbound")) {
                        rawLabel = rawLabel.replaceAll("[\\p{Cntrl}\\p{So}]", "");

                        String lowerRawLabel = rawLabel.toLowerCase();
                        boolean rawHasCtrl = lowerRawLabel.contains("ctrl") || lowerRawLabel.contains("control");
                        boolean rawHasShift = lowerRawLabel.contains("shift");
                        boolean rawHasAlt = lowerRawLabel.contains("alt");

                        if (hasCtrl && !rawHasCtrl) result.append("CTRL+");
                        if (hasShift && !rawHasShift) result.append("SHIFT+");
                        if (hasAlt && !rawHasAlt) result.append("ALT+");

                        result.append(rawLabel);
                        return result.toString();
                    }
                }
            } catch (Exception ignored) {
    ...
    class BindListItem extends ListItem {
        ...
        public boolean shouldRemove() {
            if (!module.hasKeybind() || !module.isVisible() || hideBounded.getValue()) return true;
            if (hideActive.getValue() && module.isEnabled()) return true;
            if (hideInactive.getValue() && !module.isEnabled()) return true;
            return false;
        }
    }
